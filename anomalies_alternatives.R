# ============================================================
# 🌍 LAND USE ANOMALY PIPELINE — PRODUCTION READY (FIXED)
# ============================================================
...
library(DT)
library(htmlwidgets)
library(htmltools)    # needed for popupTableSafe()
library(webshot)
library(leaflet.extras)
# library(phantomjs)

demo_mode <- TRUE
mapviewOptions(fgb = FALSE)           # stable Leaflet rendering
options(mapview.popup = "html")       # ensure HTML popups

# If you haven't installed PhantomJS for webshot before (only once):
# webshot::install_phantomjs()

# ------------------------------------------------------------
# SAFE POPUP HELPER (replaces removed mapview::popupTable)
# ------------------------------------------------------------
...
#    NOTE: correct filename: national_plan_zones.gpkg
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 4) GET DYNAMIC WORLD DATA (2024 median, label)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 5) STANDARDIZE MODE COLUMN NAME (handles .mode/label_mode/label)
# ------------------------------------------------------------
...
# Clearer warning on unmatched zone types
...
# ------------------------------------------------------------
# 7) CENTROIDS & REVERSE GEOCODING (robust)
# ------------------------------------------------------------
...
if (nrow(anomalies_sf) == 0) {
  ...
  # Geocode on a plain data frame to avoid sf/tidygeocoder quirks
  ...
  # Ensure lon/lat are present with expected names
...
# Merge address back to full table by lon/lat
zonal_sf <- zonal_sf %>%
  left_join(
...
# ------------------------------------------------------------
# 9) INTERACTIVE MAP (Leaflet via mapview)
# ------------------------------------------------------------
base_map <- mapview(
  zones,
  zcol = "zone_type",
...
# ------------------------------------------------------------
# 10) DEMO MODE (optional extra layer)
# ------------------------------------------------------------
if (isTRUE(demo_mode)) {
  demo_pts <- st_as_sf(
    data.frame(
...
# 
# # Interactive map HTML (self-contained)
# # mapview::mapshot(
# #   combined_map,
...
# # )
# 
# mapview::mapshot2(combined_map, file = "output/anomalies_interactive_map.html")
# 
# 
# 
# 
...
# ------------------------------------------------------------
# 11) OUTPUTS — FIXED HTML EXPORT (NO PHANTOMJS NEEDED)
# ------------------------------------------------------------
...
# ✅ Planner table export remains the same
dt_widget <- datatable(
  planner_table,
  escape = FALSE,
  extensions = c('Buttons','Scroller'),
...
# try(browseURL(file.path(getwd(), "output/anomalies_planner_table.html")), silent = TRUE)
